...
async def parse_resume(user_id: Annotated[UUID, Depends(get_current_user_id)], file: UploadFile = File(...)) -> ApiResponse[ResumeParseResult]:
    """读取pdf"""
    pdf_bytes = await file.read()
    """解析pdf"""
    result =  await resume_service.parse_resume(pdf_bytes, user_id, file.filename or "resume.pdf")
