...
class InterviewSession(Base):
    ...
    job_profile_id: Mapped[int | None] = mapped_column(
        BigInteger,
        ForeignKey("interview.job_profiles.id"),
        nullable=True,
        comment="关联的岗位模板内部ID，可为空",
    )
    ...
    metadata_: Mapped[dict[str, Any]] = mapped_column(
        ...
        comment="扩展元数据，如模型配置、语言、prompt版本等",
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('init', 'asking', 'evaluating', 'finished', 'cancelled')",
            name="chk_interview_sessions_status",
        ),
        CheckConstraint(
            "max_questions > 0",
            name="chk_interview_sessions_max_questions",
        ),
        CheckConstraint(
            "total_rounds >= 0",
            name="chk_interview_sessions_total_rounds",
        ),
        CheckConstraint(
            "current_question_index >= 0",
            name="chk_interview_sessions_current_question_index",
        ),
        Index("idx_interview_sessions_user_id", "user_id"),
        Index("idx_interview_sessions_job_profile_id", "job_profile_id"),
        ...
        Index(
            "idx_interview_sessions_metadata_gin",
            "metadata",
